...
% TODO Zde by to chtělo nějaké schéma kde budou použity všechny typy vztahů. Např. e-shop: výrobce, kategorie, produkt, hodnocení produktu.
...
Definice: \(O(f(n))\) značí, že složitost algoritmu je menší než \(A+B\cdot f(n)\), kde \(A\) a \(B\) jsou konstanty. Tzn.\,\(O(1000\cdot n^2+1700 + n) = O(n^2)\) (v úvahu je brané jen chování funkce pro velké hodnoty).
...
\begin{itemize}
    \item Definice: Uspořádaná šestice \(T=(Q,\Sigma, \Gamma, \delta, q_0, q_F)\)
    \begin{itemize}
        \item \(Q\) -- množina vnitřních stavů
        \item \(\Sigma\) -- vstupní abeceda (znaky, které mohou být na pásce na začátku běhu), \(\Sigma\subseteq \Gamma\setminus \{\Delta\}\), kde \(\Delta\) je prázdný symbol
        \item \(\Gamma\) -- pásková abeceda (všechny znaky, které se na pásce mohou vyskytnout)
        \item \(\delta\) -- přechodová funkce, \(\delta:(Q\setminus \{q_F\})\times \Gamma \rightarrow Q\times (\Gamma\cup \{L,R\})\), kde \(L\) a \(R\) jsou posuvy pásky doleva nebo doprava
        \item \(q_0\) -- výchozí stav
        \item \(q_F\) -- konečný stav (někdy se také uvádí \(F\) jako množina konečných stavů)
    \end{itemize}
...
Dynamická datová struktura, kde se~odebírají prvky v~tom pořadí v~jakém byly vloženy (FIFO). Základní operace jsou stejné jako u~zásobníku. Existuje tzv. prioritní fronta, která funguje na~principu fronty ale bere z~ní podle priority.
...
\begin{table}[ht]
	...
	\begin{tabular}{|c||c|c|}\hline
		...
		vkládání  & \(\Theta\)(1)     & O(\( n \)) \\\hline
		mazání    & \(\Theta\)(1)     & O(\( n \)) \\\hline
		vyhledání & \(\Theta\)(1)     & O(\( n \)) \\\hline
	\end{tabular}
\end{table}
...
\section{Hledání kostry v grafu. Centralizovaná a paralelní varianta nalezení kostry v grafu.}

\subsection{Hledání kostry v grafu}

Kostra grafu je strom, spojující všechny vrcholy (neobsahuje cykly). Vytváří se tak, aby propojila všechny body a~celková váha hran byla co nejmenší. Hledání kostry s~co nejmenší váhou nazýváme \textit{minimal spanning tree}. Nejznámějšími algoritmy jsou Kruskalův algoritmus (centralizovaný) a~distribuovaný algoritmus (paralelní). U souvislého grafu se hledá podmnožina hran, která tvoří strom (obsahující všechny uzly) a jejíž celková váha (součet délek hran grafu) je minimální. V případě nesouvislého grafu o více komponentách, algoritmus hledá les minimálních koster, tedy minimální kostru každé komponenty.



Uplatnění:
\begin{itemize}
    \item Odstranění cyklů z grafu (např. přepínač)
    \item Návrh sítí (telefonické, elektrické, hydraulické, TV kabelové, počítačové, silniční,...)
    \item Analýza shluků (hledání shluků kvazarů a Seyfert galaxií, analyzování plísní prostorových modelů)
    \item Přibližná řešení tzv. NP-těžkých problémů (metrický TSP (Traveling Salesman Problem), Steiner stromy)
\end{itemize}

Nepřímá uplatnění:
\begin{itemize}
    \item Rakovinový výzkum
    \item Učení charakteristických rysů pro real-time verifikaci tváře
    \item Modelování lokálnosti interakce částic turbulentního toku kapalin
    \item Snižuje množství ukládaných dat pro popis aminokyselin v proteinu
\end{itemize}

\subsection{Centralizovaná a paralelní varianta nalezení kostry v grafu}
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=0.6\textwidth]{images/kruskal_graf.jpg}
	\caption{Kruskalův algoritmus}
	\label{kruskal}
\end{figure}

\paragraph{Distribuovaný algoritmus} Zde se~pracuje na~principu, že se~kostra tvoří na~každé množině zároveň. Z~každého vrcholu se~vyrazí směrem po~nejmenší hodnotě hrany. Pokud se~vrcholy na~cestě potkají, tak vytvoří množinu. Pokud se~nepotkají, nic se~neděje. Dále se~pak vysílají znovu po~nejmenší hodnotě hrany. Toto se~děje, dokud se~celá kostra nenajde (všechny uzly náleží stejné množině). Tento algoritmus se~nejčastěji využívá v~telekomunikačních sítích, kde neznáme celou topologii grafu.

\begin{figure}[htb]
	\centering
	\includegraphics[width=1\textwidth]{images/paralelni_graf.jpg}
