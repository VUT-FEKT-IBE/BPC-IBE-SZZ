...
\textbf{Feistelova síť} je konstrukt iterativního míchání bloků: $$L_i = R_{i-1}, R_i = L_{i-1} \oplus f(R_{i-1}, k_i)$$ kde $k_i$ je podklíč použitý v~$i$-té rundě a $f$ je libovolná funkce. Tento princip využívá DES: 64b bloky, 56b klíč ($+$8b parita), 16 rund.

\begin{table}[ht]
\centering
\begin{tabular}{p{1cm}|p{14cm}}
ECB & každý blok je šifrován samostatně, $m = m' \Rightarrow c = c'$ \\
CBC & $c_i = c_{i-1} \oplus k$ \\
CFB & bloková šifra je zdroj hesla pro~proudovou šifru \\
OPF & zpětná vazba z~výstupu generátoru, vlastnosti proudové šifry \\
CTR & iniciační IV je lineárně inkrementován \\
